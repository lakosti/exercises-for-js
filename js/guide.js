 ...
 * JS DOCa
 */
//null - це баг в JS він повертає тип даних об'єкт
//рядки в ДЖС не мутуються! Їх потрібно вирізати в додавати самостійно

// ~ -- Bitwise not -(x + 1) для indexOf
// 0 (перший елемент масива) => false
//-1 (немає елемента масива) => true
//-(0 + 1) => -1 => true // 0 перетворюється на - 1 і ІФ його пропускає
//-(-1 + 1) => -0 => false // -1 перетворюється на 0 і ІФ його не пропускає
// !!~  те шо було 0 = -1  === !(idx === -1) be twice not // якщо є елем
// !~  (idx === 0)  // якщо немає елемента

//push() повертає пустий масив

//length є лише у масива та строки!!
...
// join повертає рядок і його необхідно записати у змінну
